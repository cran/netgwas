...
# netsnp(): Reconstruct intra- and inter chromosomal interaction (linkage disequilibrium) #
...
if( method == "gibbs" ||  method== "approx" )
{
	if( method == "gibbs")
	{
		...
		if(is.null(rho.ratio)) rho.ratio = 0.15 
	...
	if(method == "approx")
	{
		...
		if(is.null(rho.ratio)) rho.ratio = 0.22
	...
	if( method == "npn" ){
		...
		if(verbose)
		{
			m <- paste("Reconstructing intra- and inter- chromosomal interactions network is in progress ... \n")
			cat(m, "\r")
			flush.console()
		}
		...
		if((is.null(rho)) && (is.null(n.rho)) ) n.rho  = 6
		if(! is.null(rho)) n.rho  = length(rho)
		if(is.null(rho.ratio)) rho.ratio = 0.14
		...
		if(verbose)
		{
			m <- paste("Reconstructing intra- and inter- chromosomal interactions network done. \n")
			cat(m, "\r")
			flush.console()
		}
	}
...
print.netgwas = function(x, ...){
	...
	sparsLevel <- round(sapply(1:length(x$rho), function(i) sum(x$Theta[[i]])/ncol(x$data)/(ncol(x$data)-1)),4)
